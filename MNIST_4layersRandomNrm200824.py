...
stitle = 'MNIST_4layersRandomNrm200824'
num_repeat = 2
num_epoch = 20
idim = 512
randomseed = 1
...
for irpt in range(num_repeat):
  ...
  for id in idlist:
    ...
    ######
    irmax = (idim - id)*(idim - id)
    iflg = [[0] * idim for i in range(idim)]
    #frac = ((idim - id) * (idim - id)) * fdim2
    for ir in range(irmax):
      while True:
        iy = random.randrange(idim)
        ix = random.randrange(idim)
        if (iflg[ix][iy] == 0):
            break
      #while end
      lsczw[0][ix][iy] = 0
